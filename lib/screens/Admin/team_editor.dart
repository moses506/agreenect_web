...
class _TeamSectionEditorState extends State<TeamSectionEditor> {
  ...
  final _sc           = ScrollController(); // ← auto-scroll controller
  ...
  void dispose() {
    ...
    _sc.dispose(); // ← dispose controller
    super.dispose();
  }
  ...
  void _addMember() {
    ...
    // ← Scroll to reveal the new member card after it renders
    _scrollToBottom(_sc);
  }
  ...
  void _removeMember(int i) => showDialog(
    ...
    builder: (_) => AlertDialog(
      ...
      title: const Text('Remove member?',
          style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _C.ink)),
      content: Text(
      ...
      actions: [
        TextButton(
          ...
          child: Text('Cancel', style: TextStyle(color: _C.ash)),
        ),
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Row(children: [
        Icon(err ? Icons.error_outline : Icons.check_circle_rounded,
            color: err ? _C.rose : _C.lime, size: 17),
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      ...
      child: Column(children: [
        ...
        Expanded(
          child: ListView(
            controller: _sc, // ← wire controller to the scrollable list
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Team Section',
              style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800,
                  color: _C.ink, letterSpacing: -0.5)),
          const SizedBox(height: 3),
          Text(
            '${_members.length} member${_members.length != 1 ? 's' : ''} · Manage your team profiles',
            style: TextStyle(fontSize: 13, color: _C.ash),
          ),
        ])),
        _AgBtn(label: '+ Add Member', icon: Icons.person_add_rounded,
            ghost: true, onTap: _addMember),
  ...
  Widget _buildMembersHeader() {
    return Row(children: [
      Container(width: 3, height: 18,
          decoration: BoxDecoration(color: _C.leaf, borderRadius: BorderRadius.circular(2))),
      const SizedBox(width: 10),
      Text('TEAM MEMBERS',
          style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800,
              color: _C.ash, letterSpacing: 1.5)),
      const SizedBox(width: 10),
      Container(
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const Text('No team members yet',
            style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: _C.ink)),
    ...
    return Padding(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: isExpanded ? _C.leaf.withValues(alpha: 0.5) : _C.mist,
              width: isExpanded ? 1.8 : 1),
          boxShadow: isExpanded
              ? [BoxShadow(color: _C.leaf.withValues(alpha: 0.08),
        ...
        child: Column(children: [
          ...
          InkWell(
            ...
            child: Padding(
              ...
              child: Row(children: [
                _MiniAvatar(imageUrl: m['image'] ?? '', name: name, size: 42),
                    ...
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Text(role, style: const TextStyle(
                            fontSize: 11, fontWeight: FontWeight.w600, color: _C.leaf)),
                      ),
                    ),
                ])),
                Container(
                  ...
                  child: Center(child: Text('${i + 1}',
                      style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: _C.ash))),
                ),
  ...
  Widget _buildMemberEditor(int i) {
    final m = _members[i];
    return Container(
      decoration: const BoxDecoration(border: Border(top: BorderSide(color: _C.mist))),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Container(width: 3, height: 14,
              decoration: BoxDecoration(color: _C.smoke, borderRadius: BorderRadius.circular(2))),
          const SizedBox(width: 8),
          Text('CONTACT & SOCIAL', style: TextStyle(
              fontSize: 9, fontWeight: FontWeight.w800, color: _C.ash, letterSpacing: 1.5)),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _AField(
...
class _AField extends StatelessWidget {
  ...
  const _AField({
    required this.label, this.hint, this.icon, this.ctrl, this.init,
    ...
    children: [
      RichText(text: TextSpan(
        ...
        style: const TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700,
            color: _C.ash, letterSpacing: 0.5),
        children: req
            ? const [TextSpan(text: ' *', style: TextStyle(color: _C.rose))]
            : [],
      )),
      const SizedBox(height: 6),
      TextFormField(
...
class _AgBtn extends StatelessWidget {
  ...
  final bool ghost;
  final bool danger;
  const _AgBtn({required this.label, required this.icon, required this.onTap,
      this.ghost = false, this.danger = false});

  @override
  Widget build(BuildContext context) {
    final bg = ghost ? _C.mist : danger ? _C.rose : _C.canopy;
    final fg = ghost ? _C.ink  : _C.white;
    return Material(
      color: bg,
...
class _VisToggle extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(mainAxisSize: MainAxisSize.min, children: [
    AnimatedContainer(
      ...
      child: Text(val ? 'Visible' : 'Hidden',
          style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
              color: val ? _C.leaf : _C.ash)),
    ),
    Switch(
...
class _MiniAvatar extends StatelessWidget {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        gradient: imageUrl.isEmpty
            ? const LinearGradient(colors: [_C.canopy, _C.leaf],
  ...
  Widget _initials(String t) => Container(
    decoration: const BoxDecoration(
      gradient: LinearGradient(colors: [_C.canopy, _C.leaf],
          begin: Alignment.topLeft, end: Alignment.bottomRight),
    ),
...
class _AvatarUpload extends StatefulWidget {
  ...
  const _AvatarUpload({
      required this.imageUrl, required this.memberName, required this.onUploaded});
...
class _AvatarUploadState extends State<_AvatarUpload> {
        ...
        child: Stack(alignment: Alignment.center, children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(
                  color: _C.leaf.withValues(alpha: 0.2),
  ...
  Widget _placeholder(String initials) => Container(
    ...
    child: Center(child: Text(initials,
        style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: _C.white))),
  );
}